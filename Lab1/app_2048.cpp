...
void app_2048::on_mouse_click(HWND window, int x, int y)
{
	POINT pt{ x, y };
	bool updated = false;
	// Sprawdzenie, czy klikniêcie nast¹pi³o na którymœ z pól
	for (auto& f : m_board.fields()) {
		if (PtInRect(&f.position, pt)) {
			// Prze³¹czenie koloru – jeœli domyœlny, ustaw nowy (np. CornflowerBlue), inaczej przywróæ domyœlny
			if (f.color == RGB(204, 192, 174))
				f.color = RGB(100, 149, 237); // nowy kolor
			else
				f.color = RGB(204, 192, 174); // przywrócenie domyœlnego koloru
			updated = true;
			break; // zmieniono tylko jedno pole
		}
	}
	if (updated) {
		InvalidateRect(window, nullptr, TRUE); // odœwie¿enie okna, by zmiana by³a widoczna
	}
}